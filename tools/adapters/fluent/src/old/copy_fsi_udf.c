...
void regather_read_positions(Dynamic_Thread* dt, int thread_new_size)
{
  ...
  face_t face;
  int meshID = precice_getMeshID("WetSurface");
  int all_size = precice_getReadNodesSize(meshID);
  int displID = precice_getDataID("Displacements");
  ...
  precice_readBlockVectorData(displID, all_size, all_indices, all_displ);
  precice_getReadPositions(meshID, all_size, all_indices, all_coords);
        ...
        if (NODE_POS_NEED_UPDATE(node)){
          ...
          for (j=0; j < all_size; j++){ /* Find position index in all positions */
            for (dim=0; dim < ND_ND; dim++){ /* Vector equality comp */
              ...
              if (fabs(NODE_COORD(node)[dim] - all_coords[j*ND_ND+dim]) > 1e-10){
                break;
              }
            }

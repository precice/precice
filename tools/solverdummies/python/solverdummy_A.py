...
participant_name = "dummy_Solver_One"
mesh_name = "dummy_Mesh_One"
...
while interface.is_coupling_ongoing():
    ...
    for i in range(0,n): 
        solver_Two_Data[i] = interface.read_scalar_data(solver_Two_Data_ID, data_indices[i])            
        print("solver_Two_Data = ", solver_Two_Data[i])
        print("data_indices[i] = ", data_indices[i])
        solver_One_Data[i] = solver_Two_Data[i] + 1
    ...
    for i in range(0,n):    
        ...
        print("data_indices[i] = ", data_indices[i])
        interface.write_scalar_data(solver_One_Data_ID, data_indices[i], solver_One_Data[i])

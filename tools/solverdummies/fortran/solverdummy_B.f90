PROGRAM main
  ...
  rank = 0
  commsize = 1
  CALL precicef_create('dummy_Solver_Two', 'precice-config.xml', rank, commsize)
  ...
  DO WHILE (ongoing.NE.0)
    ...
    do i = 1,n,1
      CALL precicef_read_sdata(solver_One_Data_ID, vertexID(i), solver_One_Data(i))
      print*,'solver_One_Data = ', solver_One_Data(i)
      print*,'vertexID = ', vertexID(i)
    enddo

    solver_Two_Data = solver_One_Data + 1

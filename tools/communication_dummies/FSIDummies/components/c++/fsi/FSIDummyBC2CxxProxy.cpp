#include "fsi/FSIDummyBC2CxxProxy.h"
#ifdef _WIN32
	#include <winsock2.h>
	#include <ws2tcpip.h>
...
void invoker_create_instance(void** ref,int,int,char*,char*
     ...
     *ref = new fsi::FSIDummyBImplementation();
}
...
void parallel_master_invoker_transferCoordinates(void** ref,int newsockfd, int buffer_size,char* rcvBuffer, char* sendBuffer
#ifdef Parallel
,MPI_Comm communicator, int methodId
#endif
){
...
void parallel_worker_invoker_transferCoordinates(void** ref
#ifdef Parallel
,MPI_Comm newsockfd
#endif
){
...
std::string retrieveSocketAddress(){
	...
	assert(_sockfd>=0);
	const char* network_interface = getenv("FSI_FSIDUMMYB_NET_INTERFACE");
...
void* parallel_deamon_run(void* daemon_args){
	parallel_worker_loop(((FSI_FSIDUMMYB_arg*)daemon_args)->ref,((FSI_FSIDUMMYB_arg*)daemon_args)->communicator);
}
#endif
...
void* server_deamon_run(void* daemon_args){
      ...
      int clientfd=0;
#endif
      accept_on_server(((FSI_FSIDUMMYB_arg*)daemon_args)->daemon_serverfd,clientfd);
      std::cout<<"server accepted"<<std::endl;
      socket_worker_loop(
      	((FSI_FSIDUMMYB_arg*)daemon_args)->ref,
      	clientfd,
      	((FSI_FSIDUMMYB_arg*)daemon_args)->buffer_size
#ifdef Parallel
		,((FSI_FSIDUMMYB_arg*)daemon_args)->communicator
#endif      	
      	);
}
void startMPIDaemon(FSI_FSIDUMMYB_arg& arg){
#ifdef Parallel	 
int rank = -1;
MPI_Comm_rank(MPI_COMM_WORLD,&rank);
if(rank>0){
...
void startSocketDaemons(FSI_FSIDUMMYB_arg& arg){
...
void initialiseENV(FSI_FSIDUMMYB_arg& arg){
          const char* client_port = getenv("FSI_FSIDUMMYB_PORT");
          const char* daemon_port = getenv("FSI_FSIDUMMYB_DAEMON_PORT");
          const char* buffer_size = getenv("FSI_FSIDUMMYB_BUFFER_SIZE");
          const char* hostname = getenv("FSI_FSIDUMMYB_HOSTNAME");
          const char* java_client_flag = getenv("FSI_FSIDUMMYB_JAVA");
          const char* number_of_workers = getenv("FSI_FSIDUMMYB_WORKERS");
          const char* xmlFile = getenv("FSI_FSIDUMMYB_XML");
          if(buffer_size!=NULL)
...
void initialiseXMLDaemons(FSI_FSIDUMMYB_arg& arg){
    int rank=0;
    ...
    if(arg.xml!=NULL){
          ...
          {
            if(strcmp(e->Attribute("name"),"fsi.FSIDummyB")==0){
                 int port;
...
void initialiseXMLConnections(FSI_FSIDUMMYB_arg& arg){
    int rank=0;
...
void initialiseParallel(FSI_FSIDUMMYB_arg& arg){
#ifdef Parallel
     int rank = -1 ;
     int comm_size;
     std::stringstream st;
...
#ifdef _WIN32
void INITIALISE(FSI_FSIDUMMYB_arg& arg,bool joinable){
#else
void initialise_(FSI_FSIDUMMYB_arg& arg,bool joinable){
#endif
...
#ifdef _WIN32
void SOCKET_LOOP(FSI_FSIDUMMYB_arg& arg,bool joinable){
#else
void socket_loop_(FSI_FSIDUMMYB_arg& arg,bool joinable){
#endif
  if(arg.java_client_flag)       

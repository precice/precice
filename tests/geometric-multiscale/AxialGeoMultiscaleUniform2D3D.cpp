...
BOOST_AUTO_TEST_CASE(AxialGeoMultiscaleUniform2D3D)
{
  ...
  if (context.isNamed("Fluid2D")) {
    ...
    if (cplInterface.requiresInitialData()) {
      valueDataA[0] = 6.0;
      valueDataA[1] = 8.0;
      cplInterface.writeData(meshName, dataAName, vids2D, valueDataA);
    }
...

    {
      ...
      // - 2D vertex at x=0.5 collects from 3D vertices x=0.5,0.5 → avg(10,8) = (0,0,9)
    ...
    std::array<double, N3D * DIM> valueDataB3D = {
        ...
        0.0, 0.0, 10.0, // 3D vertex x=0.5 (y=0.2)
        0.0, 0.0, 8.0,  // 3D vertex x=0.5 (y=0.5)
    };
    ...
    // - 2D vertex x=0.5 (8) → 3D vertices x=0.5 and x=0.5 → 8, 8

...
BOOST_AUTO_TEST_CASE(AxialGeoMultiscaleVectorParabolic2D3D_Square)
{
  PRECICE_TEST();
  ...
  if (context.isNamed("Fluid2D")) {
    ...
    Vector3d w0(0.0, -0.5, 0.0);
    Vector3d w1(0.5, -0.5, 0.0);
    Vector3d w2(0.0, 0.5, 0.0);
    Vector3d w3(0.5, 0.5, 0.0);
    ...
    const double b2_h = 1.0 - 0.5 * 0.5; // 0.75

    const double e0 = u0 * pref * std::pow(b2_0, m);
    const double e1 = u0 * pref * std::pow(b2_h, m);
    const double e2 = u1 * pref * std::pow(b2_0, m);
    const double e3 = u1 * pref * std::pow(b2_h, m);

    Vector3d exp0(0.0, 0.0, e0);
    Vector3d exp1(0.0, 0.0, e1);
    Vector3d exp2(0.0, 0.0, e2);
    Vector3d exp3(0.0, 0.0, e3);

    const double tol = 1e-11;

    BOOST_TEST((vOut0 - exp0).norm() <= tol);
    BOOST_TEST((vOut1 - exp1).norm() <= tol);
    BOOST_TEST((vOut2 - exp2).norm() <= tol);

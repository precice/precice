...
BOOST_AUTO_TEST_CASE(AccessReceivedMeshOverlapNoWrite)
{
  ...
  const std::vector<double> boundingBoxSecondary      = std::vector<double>{0.0, 1.0, 3.0, 7};
  const std::vector<double> expectedPositionSecondary = std::vector<double>{0.0, 3.0, 0.0, 4.0, 0.0, 5.0};
  const std::vector<double> writeDataSecondary        = std::vector<double>({4, 5, 6});
  const std::vector<double> expectedReadDataSecondary = std::vector<double>({3, 4, 5});
  runTestAccessReceivedMesh(context, boundingBoxSecondary, writeDataSecondary, expectedPositionSecondary, expectedReadDataSecondary, 1);
}

...
/**
 * @brief Test to run a simple coupling with first order waveform subcycling. Uses also first order extrapolation.
 *
 * Provides a dt argument to the read function. A first order waveform is used.
 */
BOOST_AUTO_TEST_CASE(ReadWriteScalarDataWithWaveformSamplingFirstExtrapolation)
{
  ...
  double sampleDts[5] = {0.0, dt / 4.0, dt / 2.0, 3.0 * dt / 4.0, dt};
  ...
  while (precice.isCouplingOngoing()) {
    ...
    for (int j = 0; j < nSamples; j++) {
      ...
      if (context.isNamed("SolverOne") && iterations == 0 && timewindow == 0) { // first participant uses constant extrapolation in first window.
        BOOST_TEST(readData == readFunction(time));
      } else if (context.isNamed("SolverOne") && iterations == 0 && timewindow > 0) { // first participant uses linear extrapolation in first iteration. Exactly matches actual function.

...
BOOST_AUTO_TEST_CASE(TestExplicitWithDataMultipleReadWrite)
{
  ...
  if (context.isNamed("SolverOne")) {
    ...
    while (cplInterface.isCouplingOngoing()) {
      ...
      // multiple readScalarData
      cplInterface.readScalarData(meshID, dataBID, 0, readDataB[0]);
      // expected data value received
      BOOST_TEST(5.0 == readDataB[0]);
      // change value at read destination
      readDataB[0] = -1.61;
    ...
    cplInterface.writeBlockScalarData(meshID, dataBID, 1, vertexIDs.data(), writeDataB.data());
    ...
    cplInterface.writeBlockVectorData(meshID, dataAID, 1, vertexIDs.data(), writeDataA.data());
    // changed data value sent, overwriting previous one
    writeDataA[0] = -2.32;
    writeDataA[1] = -2.32;
    writeDataA[2] = -2.32;

...
OVERRIDES = {
    ...
    ("flow-over-heated-plate-partitioned-flow", "Fluid1-Fluid-Mesh", "Pressure"): (103499.999, 103500.001),
    # Fully developed outflow: near-zero gradients keep the outlet BC well posed.
    ("flow-over-heated-plate-partitioned-flow", "Fluid1-Fluid-Mesh", "VelocityGradient"): (-1e-6, 1e-6),
    ("flow-over-heated-plate-partitioned-flow", "Fluid1-Fluid-Mesh", "FlowTemperatureGradient"): (-1e-6, 1e-6),
    ...
    ("flow-over-heated-plate-partitioned-flow", "Fluid1-Solid-Mesh", "Heat-Flux"): (-1e-3, 1e-3),
    ...
    ("partitioned-backwards-facing-step", "Fluid1-Mesh", "VelocityGradient"): (-1e-6, 1e-6),
}
...
def main():
    ...
    for t in targets:
        ...
        pairs = sorted(set(re.findall(
            r'read-data\s+name="([^"]+)"\s+mesh="([^"]+)"', text)))
        if not pairs:
        ...
        for name, mesh in pairs:
            ...
            lines.append(
                f'  <mocked-data mesh="{mesh}" data="{name}" mode="random">')

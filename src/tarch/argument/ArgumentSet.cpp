...
precice::logging::Logger tarch::argument::ArgumentSet::_log("ArgumentSet()");
...
void tarch::argument::ArgumentSet::initialize(unsigned int argc, char* argv[]) {
	...
	if((argc-2) != _nrArgs) {
		preciceWarnung("initialize", "Error! Wrong number of arguments!");
...
tarch::argument::Argument tarch::argument::ArgumentSet::getArgument(const std::string argumentId) const {
	...
	print(ss);
	preciceWarnung("getArgument", ss.str());
	exit(-1);
	return _arguments[0];
}

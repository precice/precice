...
tarch::plotter::griddata::unstructured::vtk::VTKTextFileWriter::~VTKTextFileWriter() {
  if (!_writtenToFile) {
    assertion1( _numberOfVertices==0, "Still vertices in vtk writer pipeline. Maybe you forgot to call writeToFile() on a data vtk writer?" );
    assertion1( _numberOfCells==0, "Still cells in vtk writer pipeline. Maybe you forgot to call writeToFile() on a data vtk writer?" );
    assertion1( _numberOfCellEntries==0, "Still cell entries in vtk writer pipeline. Maybe you forgot to call writeToFile() on a data vtk writer?" );
  }
}

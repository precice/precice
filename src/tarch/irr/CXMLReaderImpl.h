...
class CXMLReaderImpl : public IIrrXMLReader<char_type, superclass> {
  ...
  /**
   * Added by Tobias Weinzierl 30.8.2006, modified by Tobias Neckel 27.04.2007
   *
   * This method returns a double value of an attribute. It is possible to
   * specify fractions (such as "1/9" instead of "0.1111111"). If some problems
   * exist, std::numeric_limits<double>::signaling_NaN() is returned. It is
   * assumed that std::numeric_limits<double>::has_signaling_NaN() is true in
   * order to do so.
   *
   * This method calls getAttributeValueAsDouble(const std::string& value) with
   * the generated string.
   *
   * @param name Name of the desired attribute value.
   *
   * @return Double value of the desired attribute name.
   */
	double getAttributeValueAsDouble(const char_type* name) const {
    const SAttribute* attr = getAttributeByName(name);
    if (!attr)
      return 0;

    return  IIrrXMLReader<char_type, superclass>::convertValueStringToDouble( std::string( attr->Value.c_str() ) );
  }

  /**
   * Added by Tobias Weinzierl 30.8.2006.
   */
  virtual double getAttributeValueAsDouble(int idx) const {
		const char_type* attrvalue = getAttributeValue(idx);
		if (!attrvalue)
			return 0;

    return IIrrXMLReader<char_type, superclass>::convertValueStringToDouble( std::string(attrvalue) );
	}


  virtual bool getAttributeValueAsBool(const char_type* name) const {
    const SAttribute* attr = getAttributeByName(name);
    if (!attr)
      return 0;
  ...
  virtual bool getAttributeValueAsBool(int idx) const {
    const char_type* attrvalue = getAttributeValue(idx);
    if (!attrvalue)
      return 0;
...
private:
	...
	void convertToLittleEndian(src_char_type* t)
	{
		if (sizeof(src_char_type) == 4)
		{
			...
			while(*t)
			{
				*t = static_cast<src_char_type>(((*t & 0xff000000) >> 24) |
				     ((*t & 0x00ff0000) >> 8)  |
				     ((*t & 0x0000ff00) << 8)  |
				     ((*t & 0x000000ff) << 24));
				++t;
		...
		else
		{
			...
			while(*t)
			{
				*t = static_cast<src_char_type>((*t >> 8) | (*t << 8));
				++t;
			}
		}
	}

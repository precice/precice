...
M2NConfiguration::M2NConfiguration(xml::XMLTag &parent)
{
  ...
  XMLTag::Occurrence occ = XMLTag::OCCUR_ARBITRARY;
  {
    ...
    auto attrNetwork =
        makeXMLAttribute("network", utils::networking::loopbackInterfaceName())
  ...
  auto attrFrom = XMLAttribute<std::string>("from").setDocumentation(
  ...
  auto attrTo = XMLAttribute<std::string>("to").setDocumentation("Second participant name involved in communication.");
...
m2n::PtrM2N M2NConfiguration::getM2N(const std::string &from, const std::string &to)
{
  ...
  PRECICE_ERROR("There is no m2n communication configured between participants \"" + from + "\" and \"" + to +
                "\". Please add an appropriate \"<m2n />\" tag.");
}

bool M2NConfiguration::isM2NConfigured(const std::string &from, const std::string &to)
{
  return std::any_of(std::begin(_m2ns), std::end(_m2ns), [from, to](const auto &m2nTuple) {
    return ((std::get<1>(m2nTuple) == from) && (std::get<2>(m2nTuple) == to)) ||
           ((std::get<1>(m2nTuple) == to) && (std::get<2>(m2nTuple) == from));
  });
}
  ...
  if (tag.getNamespace() == TAG) {
    ...
    if (enforceGatherScatter && useTwoLevelInit) {
      throw std::runtime_error{
          std::string{"A gather-scatter m2n communication cannot use two-level initialization. Please switch either "} +
          "\"" + ATTR_ENFORCE_GATHER_SCATTER + "\" or \"" + ATTR_USE_TWO_LEVEL_INIT + "\" off."};
    }
    if (context.size == 1 && useTwoLevelInit) {
      throw std::runtime_error{"To use two-level initialization, both participants need to run in parallel. If you "
                               "want to run in serial please switch two-level initialization off."};
    }
    ...
    if (tagName == "sockets") {
      ...
      PRECICE_ERROR(
          "Communication type \"mpi-multiple-ports\" can only be used if preCICE was compiled with MPI support "
          "enabled. "
          "Either switch to a \"sockets\" communication or recompile preCICE with \"PRECICE_MPICommunication=ON\".");
#else
#ifdef OMPI_MAJOR_VERSION
      PRECICE_WARN("preCICE was compiled with OpenMPI and configured to use <m2n:mpi-multiple-ports />, which can "
                   "cause issues in connection build-up. Consider switching to sockets if you encounter problems.");
#endif
...
#else
#ifdef OMPI_MAJOR_VERSION
      PRECICE_WARN("preCICE was compiled with OpenMPI and configured to use <m2n:{} />, which can cause issues in "
                   "connection build-up. Consider switching to sockets if you encounter problems.",
                   tagName);
#endif
...
void M2NConfiguration::checkDuplicates(const std::string &from, const std::string &to)
{
  using std::get;

...
template <typename ATTRIBUTE_T> class XMLAttribute {
  static_assert(std::is_default_constructible<ATTRIBUTE_T>::value,
...
public:
  ...
  template <class T> XMLAttribute &setOptions(std::initializer_list<T> &&options)
  {
    static_assert(std::is_convertible<T, ATTRIBUTE_T>::value,
...
private:
  ...
  template <typename VALUE_T>
  typename std::enable_if<std::is_same<VALUE_T, ATTRIBUTE_T>::value && std::is_same<VALUE_T, Eigen::VectorXd>::value,
                          void>::type
  set(ATTRIBUTE_T &toSet, const VALUE_T &setter);
};
  ...
  if (position == aAttributes.end()) {
    ...
    if (_hasValidation) {
      if (std::find(_options.begin(), _options.end(), _value) == _options.end()) {
        ...
        stream << "Invalid value \"" << _value << "\" of attribute \"" << getName() << "\": ";
        // print first
...
template <typename ATTRIBUTE_T>
template <typename VALUE_T>
typename std::enable_if<std::is_same<VALUE_T, ATTRIBUTE_T>::value && not std::is_same<VALUE_T, Eigen::VectorXd>::value,
                        void>::type
XMLAttribute<ATTRIBUTE_T>::set(ATTRIBUTE_T &toSet, const VALUE_T &setter)
{
  toSet = setter;
}
 ...
 *  @return an XMLAttribute with the above settings
 */
template <typename T> XMLAttribute<T> makeXMLAttribute(std::string name, T defaultValue)
{

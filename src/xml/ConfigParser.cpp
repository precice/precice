  ...
  for (auto &subtag : SubTags) {
    ...
    if (tagPosition == DefTags.end()) {
      ...
      if (!matches.empty() && matches.front().distance < 3) {
        ...
        PRECICE_ERROR("The configuration contains an unknown tag <{}>. Did you mean <{}>?", expectedName, utils::join(stringMatches, ">,<"));
      } else {
        PRECICE_ERROR("The configuration contains an unknown tag <{}>. Expected tags are {}.", expectedName, utils::join(names, ", "));
      }
    }

      ...
      _normal(Eigen::VectorXd::Zero(edgeOne.getDimensions()))
{
  ...
  P_ASSERT(&edge(0).vertex(_vertexMap[0]) != &edge(1).vertex(_vertexMap[1]));
  P_ASSERT(&edge(0).vertex(_vertexMap[0]) != &edge(2).vertex(_vertexMap[2]));
  P_ASSERT(&edge(1).vertex(_vertexMap[1]) != &edge(2).vertex(_vertexMap[2]));
  P_ASSERT((_vertexMap[0] == 0) || (_vertexMap[0] == 1), _vertexMap[0]);
  P_ASSERT((_vertexMap[1] == 0) || (_vertexMap[1] == 1), _vertexMap[0]);
  P_ASSERT((_vertexMap[2] == 0) || (_vertexMap[2] == 1), _vertexMap[0]);
}

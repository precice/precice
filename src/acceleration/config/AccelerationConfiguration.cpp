...
AccelerationConfiguration::AccelerationConfiguration(const mesh::PtrMeshConfiguration &meshConfig)
    : TAG("acceleration"), TAG_RELAX("relaxation"), TAG_INIT_RELAX("initial-relaxation"),
      TAG_MAX_USED_ITERATIONS("max-used-iterations"), TAG_TIME_WINDOWS_REUSED("time-windows-reused"), TAG_DATA("data"),
      TAG_FILTER("filter"), TAG_ESTIMATEJACOBIAN("estimate-jacobian"), TAG_PRECONDITIONER("preconditioner"),
      TAG_IMVJRESTART("imvj-restart-mode"), ATTR_NAME("name"), ATTR_MESH("mesh"), ATTR_SCALING("scaling"),
      ATTR_VALUE("value"), ATTR_ENFORCE("enforce"), ATTR_SINGULARITYLIMIT("limit"), ATTR_TYPE("type"),
      ATTR_BUILDJACOBIAN("always-build-jacobian"), ATTR_IMVJCHUNKSIZE("chunk-size"),
      ATTR_RSLS_REUSED_TIME_WINDOWS("reused-time-windows-at-restart"), ATTR_RSSVD_TRUNCATIONEPS("truncation-threshold"),
      ATTR_PRECOND_NONCONST_TIME_WINDOWS("freeze-after"), VALUE_CONSTANT("constant"), VALUE_AITKEN("aitken"),
      VALUE_IQNILS("IQN-ILS"), VALUE_MVQN("IQN-IMVJ"), VALUE_BROYDEN("broyden"), VALUE_QR1FILTER("QR1"),
      VALUE_QR1_ABSFILTER("QR1-absolute"), VALUE_QR2FILTER("QR2"), VALUE_CONSTANT_PRECONDITIONER("constant"),
      VALUE_VALUE_PRECONDITIONER("value"), VALUE_RESIDUAL_PRECONDITIONER("residual"),
      VALUE_RESIDUAL_SUM_PRECONDITIONER("residual-sum"), VALUE_LS_RESTART("RS-LS"), VALUE_ZERO_RESTART("RS-0"),
      VALUE_SVD_RESTART("RS-SVD"), VALUE_SLIDE_RESTART("RS-SLIDE"), VALUE_NO_RESTART("no-restart"),
      _meshConfig(meshConfig), _acceleration(), _neededMeshes(), _preconditioner(), _config()
{
...
void AccelerationConfiguration::connectTags(xml::XMLTag &parent)
{
  ...
  }
  {
    ...
    auto alwaybuildJacobian =
        makeXMLAttribute(ATTR_BUILDJACOBIAN, false)
            .setDocumentation("If set to true, the IMVJ will set up the Jacobian matrix"
                              ...
                              " the Jacobian is only build in the last iteration and the updates are computed using "
                              "(relatively) cheap MATVEC products.");
...
void AccelerationConfiguration::xmlTagCallback(const xml::ConfigurationContext &context, xml::XMLTag &callingTag)
{
...
void AccelerationConfiguration::xmlEndTagCallback(const xml::ConfigurationContext &context, xml::XMLTag &callingTag)
{
  ...
  if (callingTag.getNamespace() == TAG) {
    ...
    if (callingTag.getName() == VALUE_IQNILS || callingTag.getName() == VALUE_MVQN) {
      ...
      // if imvj restart-mode is of type RS-SVD, max number of non-const preconditioned time windows is limited by the
      // chunksize
    ...
    if (callingTag.getName() == VALUE_CONSTANT) {
      _acceleration = PtrAcceleration(new ConstantRelaxationAcceleration(_config.relaxationFactor, _config.dataIDs));
...
void AccelerationConfiguration::addCommonIQNSubtags(xml::XMLTag &tag)
{
  ...
  auto attrScaling = makeXMLAttribute(ATTR_SCALING, 1.0)
                         .setDocumentation("To improve the performance of a parallel or a multi coupling schemes, "
                                           "data values can be manually scaled. We recommend, however, to use an "
                                           "automatic scaling via a preconditioner.");
  ...
  tagFilter.setDocumentation(
      "Type of filtering technique that is used to "
      ...
      " - `QR2-filter`: en-block QR-dec with test \\\\(\\lVert v_\\text{orth} \\rVert_2 < \\epsilon * \\lVert v "
      "\\rVert_2\\\\)\n\n"
  ...
  auto attrFilterName = XMLAttribute<std::string>(ATTR_TYPE)
                            .setOptions({VALUE_QR1FILTER, VALUE_QR1_ABSFILTER, VALUE_QR2FILTER})
                            .setDocumentation("Type of the filter.");
  tagFilter.addAttribute(attrFilterName);
  tag.addSubtag(tagFilter);
}

void AccelerationConfiguration::addTypeSpecificSubtags(xml::XMLTag &tag)
{
  using namespace xml;
  if (tag.getName() == VALUE_CONSTANT) {
    ...
    tagTimeWindowsReused.setDocumentation(
    ...
    tagPreconditioner.setDocumentation(
        ...
        " can be applied. A constant preconditioner scales every acceleration data by a constant value, which you can "
        "define as"
    ...
    auto nonconstTWindows =
        makeXMLAttribute(ATTR_PRECOND_NONCONST_TIME_WINDOWS, -1)
            .setDocumentation("After the given number of time windows, the preconditioner weights "
    ...
    XMLTag tagInitRelax(*this, TAG_INIT_RELAX, XMLTag::OCCUR_ONCE);
    tagInitRelax.setDocumentation("Initial relaxation factor.");
    tagInitRelax.addAttribute(XMLAttribute<double>(ATTR_VALUE).setDocumentation("Initial relaxation factor."));
    ...
    tagIMVJRESTART.setDocumentation(
        ...
        "- `RS-ZERO`:    IMVJ runs in restart mode. After M time windows all Jacobain information is dropped, restart "
        "with no information\n"
        "- `RS-LS`:      IMVJ runs in restart mode. After M time windows a IQN-LS like approximation for the initial "
        "guess of the Jacobian is computed.\n"
    ...
    auto attrChunkSize = makeXMLAttribute(ATTR_IMVJCHUNKSIZE, 8)
                             .setDocumentation("Specifies the number of time windows M after which the IMVJ restarts, "
                                               "if run in restart-mode. Default value is M=8.");
    auto attrReusedTimeWindowsAtRestart =
        makeXMLAttribute(ATTR_RSLS_REUSED_TIME_WINDOWS, 8)
            .setDocumentation(
                "If IMVJ restart-mode=RS-LS, the number of reused time windows at restart can be specified.");
    auto attrRSSVD_truncationEps =
        makeXMLAttribute(ATTR_RSSVD_TRUNCATIONEPS, 1e-4)
    ...
    auto nonconstTWindows =
        makeXMLAttribute(ATTR_PRECOND_NONCONST_TIME_WINDOWS, -1)
            .setDocumentation("After the given number of time windows, the preconditioner weights are frozen and the "
                              "preconditioner acts like a constant preconditioner.");
    tagPreconditioner.addAttribute(nonconstTWindows);
    tag.addSubtag(tagPreconditioner);
    ...
    PRECICE_ERROR("Acceleration of type \"{}\" is unknown. Please choose a valid acceleration scheme or check the "

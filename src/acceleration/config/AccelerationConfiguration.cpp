...
void AccelerationConfiguration::connectTags(xml::XMLTag &parent)
{
  ...
  }
  {
    ...
    auto reducedQN = makeXMLAttribute(ATTR_REDUCEDQN, true)
                         .setDocumentation("If set to true, QN method will only use the last timestep when constructing the Jacobian.");
    tag.addAttribute(reducedQN);
    ...
    xml::XMLTag &                    callingTag)
{
  ...
  if (callingTag.getNamespace() == TAG) {
    ...
    if (_config.type == VALUE_IQNIMVJ || _config.type == VALUE_IQNILS)
      _config.reducedQN = callingTag.getBooleanAttributeValue(ATTR_REDUCEDQN);
  }

    ...
    const DataMap &cplData)
{
  ...
  // set the number of global rows in the QRFactorization. This is essential for the correctness in primary-secondary mode!
    ...
    const std::string &s, bool allProcs)
{
  if (not utils::MasterSlave::isParallel()) {
    ...
    // parallel acceleration, primary-secondary mode
  } else {
    if (not allProcs) {
      if (utils::MasterSlave::isMaster())
        _infostringstream << s;

/// @TODO Cleanup Remove old uncommented code and unused includes from this file
 ...
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::initialize(DataMap &cplData)
{
 ...
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::registerSolutionCoarseModelOptimization(
    DataMap &cplData)
{
  PRECICE_TRACE();
  ...
  // in the context of mannifold mapping acceleration we want _input_Xstar to be scaled according to the scaling factors
 ...
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::setDesignSpecification(
    Eigen::VectorXd &q)
{
  PRECICE_TRACE();
 ...
 *  ---------------------------------------------------------------------------------------------
 */
std::map<int, Eigen::VectorXd> MMAcceleration::getDesignSpecification(
    DataMap &cplData)
{
 ...
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::updateDifferenceMatrices(
    DataMap &cplData)
{
  PRECICE_TRACE();
 ...
 *         performAcceleration()
 *
 * @brief: performs one iteration of the manifold mapping acceleration. It steers the execution
 *         of fine and coarse model evaluations and also calls the coarse model optimization.
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::performAcceleration(
    DataMap &cplData)
{
 ...
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::computeCoarseModelDesignSpecifiaction()
{
  PRECICE_TRACE();
...
void MMAcceleration::concatenateCouplingData(
    DataMap &cplData)
{
  PRECICE_TRACE();
 ...
 *  -----------------------------------------------------------------------------------
 */
bool MMAcceleration::isSet(Eigen::VectorXd &designSpec)
{
 ...
 *         the manifold mapping acceleration. Stores new differences in F and C, clears or
 *         updates F and C according to the number of reused time steps
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::iterationsConverged(
    DataMap &cplData)
{
  PRECICE_TRACE();
  ...
  /** in each advance() cycle in the coupling scheme, either MMAcceleration::performAcceleration() or
   *  MMAcceleration::iterationsConverged() is called. Both, if and only if the coarse model has converged.
 ...
 *  ---------------------------------------------------------------------------------------------
 */
void MMAcceleration::removeMatrixColumn(
    int columnIndex)
{
...
void MMAcceleration::exportState(
    io::TXTWriter &writer)
{
}

void MMAcceleration::importState(
    io::TXTReader &reader)
{
}

int MMAcceleration::getDeletedColumns()
{
  return deletedColumns;
}

int MMAcceleration::getLSSystemCols()
{
  int cols = 0;
...
int MMAcceleration::getLSSystemRows()
{
  if (utils::MasterSlave::isMaster() || utils::MasterSlave::isSlave()) {

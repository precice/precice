    ...
    DataMap &cplData)
{
  ...
  for (int id : _secondaryDataIDs) {
    ...
    secResiduals -= data->readLastIteration();
  }
...
void IQNILSAcceleration::computeQNUpdate(Acceleration::DataMap &cplData, Eigen::VectorXd &xUpdate)
{
  ...
  for (int id : _secondaryDataIDs) {
    ...
    PRECICE_ASSERT(values.size() == data->readLastIteration().size(), values.size(), data->readLastIteration().size());
    values += data->readLastIteration();

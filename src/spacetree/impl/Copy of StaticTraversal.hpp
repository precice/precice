  ...
  RefineAllResult&        result )
{
  preciceTrace4 ( "refineAllInternal()", cellCenter, cellHalflengths,
                  refinementLimit, env.getPosition() );
      ...
      for ( int i=0; i < cell.getChildCount(); i++ ){
        ...
        if ( childCell.getPosition() == Spacetree::positionUndefined() ){
          ...
          if ( env.getPosition() != Spacetree::positionOnGeometry() ){
            preciceDebug("    Derive cell position from environment = " << env.getPosition());
...
//      preciceDebug ( "  Needs no refinement and has undefined position" );
//      result.uncachedCells.push_back(&cell);
//      result.uncachedCellCenters.push_back(cellCenter);
//    }
  }

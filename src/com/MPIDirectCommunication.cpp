                                              ...
                                              int                rankOffset)
{
  ...
  PRECICE_ASSERT(acceptorRank == 0, "The Acceptor/Primary has to be rank 0!");
...
void MPIDirectCommunication::reduceSum(precice::span<double const> itemsToSend, precice::span<double> itemsToReceive, Rank rankPrimary)
{
  PRECICE_TRACE(itemsToSend.size());
  PRECICE_ASSERT(itemsToSend.size() == itemsToReceive.size());
  MPI_Reduce(const_cast<double *>(itemsToSend.data()), itemsToReceive.data(), itemsToSend.size(), MPI_DOUBLE, MPI_SUM, rankPrimary, _commState->comm);
}
...
void MPIDirectCommunication::reduceSum(int itemToSend, int &itemsToReceive, Rank rankPrimary)
{
  PRECICE_TRACE();
  MPI_Reduce(&itemToSend, &itemsToReceive, 1, MPI_INT, MPI_SUM, rankPrimary, _commState->comm);
}
...
void MPIDirectCommunication::allreduceSum(precice::span<double const> itemsToSend, precice::span<double> itemsToReceive, Rank rankPrimary)
{
  PRECICE_TRACE(itemsToSend.size());
  PRECICE_ASSERT(itemsToSend.size() == itemsToReceive.size());
...
void MPIDirectCommunication::allreduceSum(double itemToSend, double &itemToReceive, Rank rankPrimary)
{
  PRECICE_TRACE();
  MPI_Allreduce(&itemToSend, &itemToReceive, 1, MPI_DOUBLE, MPI_SUM, _commState->comm);
}
...
void MPIDirectCommunication::allreduceSum(int itemToSend, int &itemToReceive, Rank rankPrimary)
{
  PRECICE_TRACE();
  MPI_Allreduce(&itemToSend, &itemToReceive, 1, MPI_INT, MPI_SUM, _commState->comm);
}

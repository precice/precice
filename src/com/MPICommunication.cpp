...
template <size_t> struct MPI_Select_unsigned_integer_datatype;

template <> struct MPI_Select_unsigned_integer_datatype<1> {
  static MPI_Datatype datatype;
};
MPI_Datatype MPI_Select_unsigned_integer_datatype<1>::datatype = MPI_UNSIGNED_CHAR;

template <> struct MPI_Select_unsigned_integer_datatype<2> {
  static MPI_Datatype datatype;
};
MPI_Datatype MPI_Select_unsigned_integer_datatype<2>::datatype = MPI_UNSIGNED_SHORT;

template <> struct MPI_Select_unsigned_integer_datatype<4> {
  static MPI_Datatype datatype;
};
MPI_Datatype MPI_Select_unsigned_integer_datatype<4>::datatype = MPI_UNSIGNED;

template <> struct MPI_Select_unsigned_integer_datatype<8> {
  static MPI_Datatype datatype;
};
...
void MPICommunication::send(std::string const &itemToSend, Rank rankReceiver)
{
  ...
  MPI_Send(const_cast<char *>(itemToSend.c_str()), itemToSend.size(), MPI_CHAR, rank(rankReceiver), 0,
...
void MPICommunication::send(precice::span<const int> itemsToSend, Rank rankReceiver)
{
  ...
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(const_cast<int *>(itemsToSend.data()), itemsToSend.size(), MPI_INT, rank(rankReceiver), 0,
           communicator(rankReceiver));
}
...
PtrRequest MPICommunication::aSend(precice::span<const int> itemsToSend, Rank rankReceiver)
{
  ...
  MPI_Request request;
  MPI_Isend(const_cast<int *>(itemsToSend.data()), itemsToSend.size(), MPI_INT, rank(rankReceiver), 0,
            communicator(rankReceiver), &request);
...
void MPICommunication::send(precice::span<const double> itemsToSend, Rank rankReceiver)
{
  PRECICE_TRACE(itemsToSend.size());
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(const_cast<double *>(itemsToSend.data()), itemsToSend.size(), MPI_DOUBLE, rank(rankReceiver), 0,
           communicator(rankReceiver));
}
...
PtrRequest MPICommunication::aSend(precice::span<const double> itemsToSend, Rank rankReceiver)
{
  ...
  MPI_Request request;
  MPI_Isend(const_cast<double *>(itemsToSend.data()), itemsToSend.size(), MPI_DOUBLE, rank(rankReceiver), 0,
            communicator(rankReceiver), &request);
...
void MPICommunication::send(double itemToSend, Rank rankReceiver)
{
  PRECICE_TRACE(itemToSend, rankReceiver);
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(&itemToSend, 1, MPI_DOUBLE, rank(rankReceiver), 0, communicator(rankReceiver));
}
...
void MPICommunication::send(int itemToSend, Rank rankReceiver)
{
  PRECICE_TRACE(itemToSend, rankReceiver);
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(&itemToSend, 1, MPI_INT, rank(rankReceiver), 0, communicator(rankReceiver));
}
...
void MPICommunication::send(bool itemToSend, Rank rankReceiver)
{
  PRECICE_TRACE(itemToSend, rankReceiver);
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(&itemToSend, 1, MPI_BOOL, rank(rankReceiver), 0, communicator(rankReceiver));
}
...
PtrRequest MPICommunication::aSend(const bool &itemToSend, Rank rankReceiver)
{
  ...
  MPI_Request request;
  MPI_Isend(const_cast<bool *>(&itemToSend), 1, MPI_BOOL, rank(rankReceiver), 0, communicator(rankReceiver), &request);
...
void MPICommunication::receive(std::string &itemToReceive, Rank rankSender)
{
  ...
  MPI_Recv(const_cast<char *>(itemToReceive.data()), length, MPI_CHAR, rank(rankSender), 0, communicator(rankSender),
           MPI_STATUS_IGNORE);
...
void MPICommunication::receive(precice::span<int> itemsToReceive, Rank rankSender)
{
  ...
  MPI_Status status;
  MPI_Recv(itemsToReceive.data(), itemsToReceive.size(), MPI_INT, rank(rankSender), 0, communicator(rankSender),
           &status);
}
...
void MPICommunication::receive(precice::span<double> itemsToReceive, Rank rankSender)
{
  ...
  MPI_Status status;
  MPI_Recv(itemsToReceive.data(), itemsToReceive.size(), MPI_DOUBLE, rank(rankSender), 0, communicator(rankSender),
           &status);
}
...
PtrRequest MPICommunication::aReceive(precice::span<double> itemsToReceive, Rank rankSender)
{
  ...
  MPI_Request request;
  MPI_Irecv(itemsToReceive.data(), itemsToReceive.size(), MPI_DOUBLE, rank(rankSender), 0, communicator(rankSender),
            &request);
...
void MPICommunication::receive(double &itemToReceive, Rank rankSender)
{
  ...
  MPI_Status status;
  MPI_Recv(&itemToReceive, 1, MPI_DOUBLE, rank(rankSender), 0, communicator(rankSender), &status);
  PRECICE_DEBUG("Received {} from rank {}", itemToReceive, rankSender);
}
...
void MPICommunication::receive(int &itemToReceive, Rank rankSender)
{
  ...
  MPI_Status status;
  MPI_Recv(&itemToReceive, 1, MPI_INT, rank(rankSender), 0, communicator(rankSender), &status);
  PRECICE_DEBUG("Received {} from rank {}", itemToReceive, rankSender);
}
...
PtrRequest MPICommunication::aReceive(int &itemToReceive, Rank rankSender)
{
  ...
  MPI_Request request;
  MPI_Irecv(&itemToReceive, 1, MPI_INT, rank(rankSender), 0, communicator(rankSender), &request);
...
void MPICommunication::receive(bool &itemToReceive, Rank rankSender)
{
  ...
  MPI_Status status;
  MPI_Recv(&itemToReceive, 1, MPI_BOOL, rank(rankSender), 0, communicator(rankSender), &status);
  PRECICE_DEBUG("Received {} from rank {}", itemToReceive, rankSender);
}
...
PtrRequest MPICommunication::aReceive(bool &itemToReceive, Rank rankSender)
{
  ...
  MPI_Request request;
  MPI_Irecv(&itemToReceive, 1, MPI_BOOL, rank(rankSender), 0, communicator(rankSender), &request);
...
void MPICommunication::send(std::vector<int> const &v, Rank rankReceiver)
{
  ...
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(const_cast<int *>(v.data()), v.size(), MPI_INT, rank(rankReceiver), 0, communicator(rankReceiver));
}
...
void MPICommunication::receive(std::vector<int> &v, Rank rankSender)
{
  ...
  v.resize(length);
  MPI_Recv(v.data(), length, MPI_INT, rank(rankSender), 0, communicator(rankSender), MPI_STATUS_IGNORE);
}
...
void MPICommunication::send(std::vector<double> const &v, Rank rankReceiver)
{
  PRECICE_TRACE(rankReceiver);
  rankReceiver = adjustRank(rankReceiver);
  MPI_Send(const_cast<double *>(v.data()), v.size(), MPI_DOUBLE, rank(rankReceiver), 0, communicator(rankReceiver));
}
...
void MPICommunication::receive(std::vector<double> &v, Rank rankSender)
{
  ...
  v.resize(length, '\0');
  MPI_Recv(v.data(), length, MPI_DOUBLE, rank(rankSender), 0, communicator(rankSender), MPI_STATUS_IGNORE);
}

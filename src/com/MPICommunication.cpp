...
{
  P_TRACE(rankReceiver, itemsToSend.size(), itemsToSend);
  rankReceiver = rankReceiver - _rankOffset;
...
void MPICommunication::receive(std::string &itemToReceive, int rankSender)
{
  ...
  MPI_Recv(const_cast<char *>(itemToReceive.data()),
           ...
           MPI_STATUS_IGNORE);
  P_DEBUG("Received \"" << itemToReceive << "\" from rank " << rankSender);
}

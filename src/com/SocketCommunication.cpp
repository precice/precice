                                           ...
                                           int                acceptorRank)
{
  ...
  try {
    ...
    P_CHECK(not ipAddress.empty(), "Network \"" << _networkName << "\" not found for socket connection!");
    ...
    P_DEBUG("Accept connection at " << address);
    ...
    do {
      ...
      P_DEBUG("Accepted connection at " << address);
      _isConnected = true;
      ...
      P_CHECK(_sockets.count(requesterRank) == 0,
    ...
    P_ERROR("Accepting connection at " << address << " failed: " << e.what());
  }
                                                   ...
                                                   int                requesterCommunicatorSize)
{
  ...
  try {
    ...
    P_DEBUG("Accepting connection at " << address);
                                            ...
                                            int                requesterCommunicatorSize)
{
  ...
  P_DEBUG("Request connection to " << address);
  ...
  try {
    ...
    P_DEBUG("Requested connection to " << address);
    ...
    P_ERROR("Requesting connection to " << address << " failed: " << e.what());
  }
                                                    ...
                                                    
{
  ...
  for (auto const & acceptorRank : acceptorRanks) {
    ...
    try {
      ...
      P_DEBUG("Requesting connection to " << ipAddress << ", port " << portNumber);
      ...
      P_DEBUG("Requested connection to " << address << ", rank = " << acceptorRank);
...
{
  P_TRACE(itemToSend, rankReceiver);
...
std::string SocketCommunication::getIpAddress()
{
  ...
  while (itNameInterface && itNameInterface->if_name) {
    P_CHECK(strlen(itNameInterface->if_name) < IFNAMSIZ,
    ...
    if (ioctl(querySocket, SIOCGIFADDR, &request) >= 0) {
      P_DEBUG(itNameInterface->if_name
      ...
      P_CHECK(strcmp(itNameInterface->if_name, _networkName.c_str()) != 0,

...
void CommunicateMesh::sendMesh(
    ...
    int               rankReceiver)
{
  P_TRACE(mesh.getName(), rankReceiver);
...
void CommunicateMesh::receiveMesh(
    ...
    int         rankSender)
{
  P_TRACE(mesh.getName(), rankSender);
  int dim = mesh.getDimensions();
  ...
  P_DEBUG("Number of vertices to receive: " << numberOfVertices);
  ...
  P_DEBUG("Number of edges to receive: " << numberOfEdges);
  if (numberOfEdges > 0) {

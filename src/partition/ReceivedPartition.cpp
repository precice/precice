...
void ReceivedPartition::compute()
{
  ...
  if (not utils::MasterSlave::isSlave()) {
    P_CHECK(_fromMapping || _toMapping,
  ...
  if (_geometricFilter == FILTER_FIRST) { //pre-filter-post-filter
    ...
    P_INFO("Pre-filter mesh " << _mesh->getName() << " by bounding-box");
    ...
    if (utils::MasterSlave::isSlave()) {
      ...
      if(areProvidedMeshesEmpty()) {
        ...
        P_CHECK(not _mesh->vertices().empty(), msg);
      }
      ...
      for (int rankSlave = 1; rankSlave < utils::MasterSlave::getSize(); rankSlave++) {
        ...
        P_DEBUG("From slave " << rankSlave << ", bounding mesh: " << _bb[0].first
      ...
      _mesh->computeState();
      P_DEBUG("Master mesh after filtering, #vertices " << _mesh->vertices().size());
    ...
    P_INFO("Broadcast mesh " << _mesh->getName());
    ...
    if (_geometricFilter == BROADCAST_FILTER) {
      ...
      P_INFO("Filter mesh " << _mesh->getName() << " by bounding-box");
      ...
      if(areProvidedMeshesEmpty()) {
        std::string msg = "The re-partitioning completely filtered out the mesh " + _mesh->getName() + " received on this rank at the coupling interface. "
          ...
          "If your geometry setup is correct and if you have very different mesh resolutions on both sides, increasing the safety-factor "
          "of the decomposition strategy might be necessary.";
        P_CHECK(not filteredMesh.vertices().empty(), msg);
      }

      P_DEBUG("Bounding box filter, filtered from " << _mesh->vertices().size() << " vertices to " << filteredMesh.vertices().size() << " vertices.");
      _mesh->clear();
      _mesh->addMesh(filteredMesh);
      _mesh->computeState();
      e2.stop();
  ...
  P_DEBUG("Tag vertices for filtering: 1st round.");
  ...
  P_DEBUG("Create owner information.");
  createOwnerInformation();

  // (4) Tag vertices 2nd round (what should be filtered out)
  P_DEBUG("Tag vertices for filtering: 2nd round.");
  if (_fromMapping)
  ...
  P_INFO("Filter mesh " << _mesh->getName() << " by mappings");
  Event e5("partition.filterMeshMappings" + _mesh->getName(), precice::syncMode);
  mesh::Mesh filteredMesh("FilteredMesh", _dimensions, _mesh->isFlipNormals());
  filterMesh(filteredMesh, false);
  P_DEBUG("Mapping filter, filtered from " << _mesh->vertices().size() << " vertices to " << filteredMesh.vertices().size() << " vertices.");
  _mesh->clear();
  _mesh->addMesh(filteredMesh);
  _mesh->computeState();
  e5.stop();
  ...
  P_INFO("Feedback distribution for mesh " << _mesh->getName());
...
void ReceivedPartition::filterMesh(mesh::Mesh &filteredMesh, const bool filterByBB)
{
  ...
  P_DEBUG("Bounding mesh. #vertices: " << _mesh->vertices().size()
  ...
  P_DEBUG("Filtered mesh. #vertices: " << filteredMesh.vertices().size()
...
void ReceivedPartition::prepareBoundingBox()
{
  ...
  for (int d = 0; d < _dimensions; d++) {
    ...
    P_DEBUG("Merged BoundingBox, dim: " << d << ", first: " << _bb[d].first << ", second: " << _bb[d].second);
  }
}
...
void ReceivedPartition::createOwnerInformation()
{
  ...
  if (utils::MasterSlave::isSlave()) {
    ...
    if (numberOfVertices != 0) {
      ...
      P_DEBUG("My tags: " << tags);
      P_DEBUG("My global IDs: " << globalIDs);
      ...
      P_DEBUG("My owner information: " << ownerVec);
    ...
    for (int rank = 1; rank < utils::MasterSlave::getSize(); rank++) {
      ...
      P_DEBUG("Rank " << rank << " has " << localNumberOfVertices << " vertices.");
      ...
      if (localNumberOfVertices != 0) {
        ...
        P_DEBUG("Rank " << rank << " has this tags " << slaveTags[rank]);
        P_DEBUG("Rank " << rank << " has this global IDs " << slaveGlobalIDs[rank]);
        bool atInterface = false;
    ...
    // Master data
    P_DEBUG("My owner information: " << slaveOwnerVecs[0]);
    setOwnerInformation(slaveOwnerVecs[0]);

#ifndef NDEBUG
    for (size_t i = 0; i < globalOwnerVec.size(); i++) {
      if (globalOwnerVec[i] == 0) {
        P_DEBUG("The Vertex with global index " << i << " of mesh: " << _mesh->getName()
    ...
    if (filteredVertices)
      P_WARN(filteredVertices << " of " << _mesh->vertices().size()

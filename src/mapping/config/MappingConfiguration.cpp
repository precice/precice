    ...
    : _meshConfig(std::move(meshConfiguration))
{
  ...
  {
    XMLTag tag(*this, VALUE_RBF_WU_C0, occ, TAG);
    tag.setDocumentation("Local radial-basis-function mapping based on the C0-polynomial RBF.");
    tag.addAttribute(attrSupportRadius);
    tags.push_back(tag);
  }
  {
    XMLTag tag(*this, VALUE_RBF_WU_C2, occ, TAG);
    tag.setDocumentation("Local radial-basis-function mapping based on the C6-polynomial RBF.");
    tag.addAttribute(attrSupportRadius);
    tags.push_back(tag);
  }
  {
    XMLTag tag(*this, VALUE_RBF_WU_C4, occ, TAG);
    tag.setDocumentation("Local radial-basis-function mapping based on the C6-polynomial RBF.");
    tag.addAttribute(attrSupportRadius);
    tags.push_back(tag);
  }
    ...
    xml::XMLTag &                    tag)
{
  ...
  if (tag.getNamespace() == TAG) {
    ...
    if (type == VALUE_RBF_GAUSSIAN || type == VALUE_RBF_INV_MULTIQUADRICS || type == VALUE_RBF_MULTIQUADRICS || type == VALUE_RBF_CTPS_C2 || type == VALUE_RBF_CPOLYNOMIAL_C0 || type == VALUE_RBF_CPOLYNOMIAL_C6 || type == VALUE_RBF_WU_C0 || type == VALUE_RBF_WU_C2 || type == VALUE_RBF_WU_C4 || type == VALUE_RBF_CPOLYNOMIAL_C2 || type == VALUE_RBF_CPOLYNOMIAL_C4) {
    ...
    Preallocation                    preallocation) const
{
  ...
  if (rbfType == RBFType::EIGEN) {
    ...
    } else if (type == VALUE_RBF_WU_C0) {
      PRECICE_ASSERT(rbfParameter.type == RBFParameter::Type::SupportRadius)
      configuredMapping.mapping = PtrMapping(
          new RadialBasisFctMapping<CompactWuC0>(
              constraintValue, dimensions, CompactWuC0(rbfParameter.value), {{xDead, yDead, zDead}}, polynomial));
    } else if (type == VALUE_RBF_WU_C2) {
      PRECICE_ASSERT(rbfParameter.type == RBFParameter::Type::SupportRadius)
      configuredMapping.mapping = PtrMapping(
          new RadialBasisFctMapping<CompactWuC2>(
              constraintValue, dimensions, CompactWuC2(rbfParameter.value), {{xDead, yDead, zDead}}, polynomial));
    } else if (type == VALUE_RBF_WU_C4) {
      PRECICE_ASSERT(rbfParameter.type == RBFParameter::Type::SupportRadius)
      configuredMapping.mapping = PtrMapping(
          new RadialBasisFctMapping<CompactWuC4>(
              constraintValue, dimensions, CompactWuC4(rbfParameter.value), {{xDead, yDead, zDead}}, polynomial));
    } else {

...
public:
  ...
  /// Evaluate the interpolant for a given vertex
  double getInterpolatedValue(const VertexID id, unsigned int direction) const;
...
private:
  ...
  std::array<Eigen::VectorXd, 3> resultVector;
...
double Partition<RADIAL_BASIS_FUNCTION_T>::getInterpolatedValue(const VertexID id, unsigned int direction) const
{
  ...
  PRECICE_ASSERT(outputIDs.contains(id), id);
  PRECICE_ASSERT(resultVector[direction].size() > outputIDs.index_of(outputIDs.find(id)));
  PRECICE_ASSERT(direction < resultVector.size());
  return resultVector[direction](outputIDs.index_of(outputIDs.find(id)));
}

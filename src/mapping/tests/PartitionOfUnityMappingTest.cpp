...
{
  PRECICE_TEST(1_rank);
  mapping::PartitionOfUnityMapping<CompactPolynomialC0> consistentMap2D(Mapping::CONSISTENT, 2, 3., Polynomial::SEPARATE, 5, 0.4, false);
                     ...
                     bool                   meshIsSmaller       = false)
{
  ...
  inMesh->setVertexOffsets({8, 8, 8, 8});
  io::ExportVTU exporter;
  exporter.doExport("pouVertices", "exports", *inMesh.get());

...
{
  PRECICE_MASTER_ONLY {
    testMethod(testConservativeIncremental);
    testMethod(testConsistentIncremental);
    testMethod(testConservativeNonIncremental);
    testMethod(testConsistentNonIncremental);
  }
}

void NearestProjectionMappingTest:: testConservativeIncremental()
{
  preciceTrace ( "testConservativeIncremental()" );
  ...
  inMesh->createVertex ( Vector2D(0.5, 0.5) );
  inMesh->allocateDataValues();
  double value = 1.0;
  inData->values()[0] = value;
  mapping.computeMapping();
  mapping.map ( inDataID, outDataID );
  utils::DynVector& values = outData->values();
  validateNumericalEquals ( values[0], value * 0.5 );
  validateNumericalEquals ( values[1], value * 0.5 );
  ...
  inMesh->vertices()[0].setCoords ( Vector2D(-0.5, -0.5) );
  mapping.computeMapping();
  mapping.map ( inDataID, outDataID );
  ...
  inMesh->vertices()[0].setCoords ( Vector2D(1.5, 1.5) );
  mapping.computeMapping();
  mapping.map ( inDataID, outDataID );
  validateNumericalEquals ( values[0], value * 1.5 );
...
void NearestProjectionMappingTest:: testConsistentIncremental()
{
  preciceTrace("testConsistentIncremental()");
  using namespace mesh;
  using utils::Vector2D;
  int dimensions = 2;
  ...
  int outDataID = outData->getID();
  Vertex& vertex = outMesh->createVertex(Vector2D(0.0));
  outMesh->allocateDataValues();
  ...
  // Map values from middle of edge. Expect mapped value to be the sum of
  // half of vertex1's value and half of vertex2's value.
  vertex.setCoords(Vector2D(0.5, 0.5));
  mapping.computeMapping();
  mapping.map(inDataID, outDataID);
  double value = outData->values()[0];
  validateEquals(mapping.hasComputedMapping(), true);
  validateNumericalEquals(value, (valueVertex1 + valueVertex2) * 0.5);

  // Map values from below edge. Expect mapped value to be the full value of
  // vertex1, since vertex2 should not be taken into account.
  vertex.setCoords(Vector2D(-0.5, -0.5));
  assign(outData->values()) = 0.0;
  mapping.computeMapping();
  mapping.map(inDataID, outDataID);
  value = outData->values()[0];
  validateNumericalEquals ( value, valueVertex1 );

  // Do the same thing from above, expect the full value of vertex2 now.
  vertex.setCoords ( Vector2D(1.5, 1.5) );
  assign(outData->values()) = 0.0;
  mapping.computeMapping();
  mapping.map ( inDataID, outDataID );
  value = outData->values()[0];
  validateNumericalEquals ( value, valueVertex2 );
}

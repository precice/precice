  ...
  mesh::Mesh&        mesh)
{
  ...
  if (_parallelWrite) { // if parallel write enabled
    if (_isProgramParallel) { // if precice in parallel mode
      ...
      writeSubFile(filename + std::to_string(rank) + ".vtu", mesh);
    } else {
  ...
  mesh::Mesh&        mesh)
{
  ...
  for (int i = 0; i < _scalarDataNames.size(); ++i) {
    outMasterFile << "         <PDataArray type=\"Int32\" Name=\""<< _scalarDataNames[i] << "\" NumberOfComponents=\"" << 1 << "\"/>" << std::endl;
  }

  for (int i = 0; i < _vectorDataNames.size(); ++i) {
    outMasterFile << "         <PDataArray type=\"Int32\" Name=\""<< _vectorDataNames[i] << "\" NumberOfComponents=\"" << _vectorDataDimensions[i] << "\"/>" << std::endl;
  }
...
(
  ...
  mesh::Mesh&        mesh)
{
  int rank = utils::Parallel::getLocalProcessRank(); // process rank
  ...
  std::string fullSubFilename(filename);
  //utils::checkAppendExtension(fullMasterFilename, ".pvtu");
  ...
  mesh::Mesh&    mesh)
{
  ...
  for (mesh::PtrData data : mesh.data()) { // Plot vertex data
	...
	int dataDimensions = data->getDimensions();
	std::string dataName(data->getName());
	outFile << "            <DataArray type=\"Int32\" Name=" << dataName << " NumberOfComponents=\"" << dataDimensions << "\" format=\"ascii\">" << std::endl;
	if(dataDimensions > 1) {

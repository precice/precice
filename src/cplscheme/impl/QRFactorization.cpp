...
bool QRFactorization::insertColumn(int k, const Eigen::VectorXd &vec, double singularityLimit)
{
  ...
  if (rho_orth <= std::numeric_limits<double>::min() || err < 0) {
    P_DEBUG("The ratio ||v_orth|| / ||v|| is extremely small and either the orthogonalization process of column v failed or the system is quadratic.");
  ...
  if (applyFilter && (rho0 * singularityLimit > rho_orth)) {
    P_DEBUG("discarding column as it is filtered out by the QR2-filter: rho0*eps > rho_orth: " << rho0 * singularityLimit << " > " << rho_orth);
    _cols--;
    return false;
  }
    ...
    int              colNum)
{
  ...
  while (!termination) {
    ...
    if (_globalRows == colNum) {
      P_WARN("The least-squares system matrix is quadratic, i.e., the new column cannot be orthogonalized (and thus inserted) to the LS-system.\nOld columns need to be removed.");
    ...
    if (rho1 * _theta <= rho0 + _omega * norm_coefficients) {
      ...
      if (k >= 4) {
        P_WARN("Matrix Q is not sufficiently orthogonal. Failed to rorthogonalize new column after 4 iterations. New column will be discarded. The least-squares system is very bad conditioned and the quasi-Newton will most probably fail to converge.");
        return -1;
      }
      rho0 = rho1;
  ...
  while (!termination) {
    ...
    if (rho1 * _theta <= rho0 + _omega * t) {
      ...
      if (!restart && rho1 <= rho * _sigma) {
        P_WARN("The new column is in the range of Q, thus not possible to orthogonalize. Try to insert a unit vector that is orthogonal to the columns space of Q.");
        //P_DEBUG("[QR-dec] - reorthogonalization");
        if (_fstream_set)

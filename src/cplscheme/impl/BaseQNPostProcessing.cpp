    ...
    DataMap &cplData)
{
  ...
  */
  ...
  if (utils::MasterSlave::isMaster() || utils::MasterSlave::isSlave()) {
    ...
    P_DEBUG("Number of unknowns at the interface (global): " << _dimOffsets.back());
    ...
    DataMap &cplData)
{
  ...
  if (_firstIteration && (_firstTimeStep || _forceInitialRelaxation)) {
    ...
    P_DEBUG("   Update Difference Matrices");
    ...
    DataMap &cplData)
{
  ...
  */
  ...
  if (_firstIteration && (_firstTimeStep || _forceInitialRelaxation)) {
    P_DEBUG("   Performing underrelaxation");
    ...
    P_DEBUG("   Performing quasi-Newton Step");
    ...
    if (not _firstTimeStep && (getLSSystemCols() < 1) && (_timestepsReused == 0) && not _forceInitialRelaxation) {
      P_DEBUG("   Last time step converged after one iteration. Need to restore the matrices from backup.");
    ...
    if (std::isnan(utils::MasterSlave::l2norm(xUpdate))) {
      P_ERROR("The coupling iteration in time step " << tSteps << " failed to converge and NaN values occurred throughout the coupling process. ");
    }
  }

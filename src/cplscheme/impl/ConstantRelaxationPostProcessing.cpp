...
ConstantRelaxationPostProcessing::ConstantRelaxationPostProcessing(
  ...
  : _relaxation(relaxation),
    _dataIDs(dataIDs)
{
  P_CHECK((relaxation > 0.0) && (relaxation <= 1.0),
...
{
  P_CHECK(_dataIDs.empty() || utils::contained(*_dataIDs.begin(), cplData),
...
void ConstantRelaxationPostProcessing::setDesignSpecification(Eigen::VectorXd &q)
{
  ...
  P_ERROR("Design specification for constant relaxation is not supported yet.");
}
}
}

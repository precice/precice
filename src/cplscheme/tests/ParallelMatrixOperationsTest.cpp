...
void ParallelMatrixOperationsTest:: run ()
{
  ...
  if (Par::getCommunicatorSize() > 3){
    ...
    if (Par::getProcessRank() <= 3){
      ...
      testMethod (testParallelMatrixMatrixOp);
...
void ParallelMatrixOperationsTest::testParVectorOperations()
{
  ...
  validate (math::equals(res1, 10.));
  validate (math::equals(ires2, 10));
  validate (math::equals(res2[0], 10.));
  validate (math::equals(res2[1], 10.));

  if(utils::MasterSlave::_masterMode){
    validate (math::equals(res3[0], 10.));
    validate (math::equals(res3[1], 10.));
    validate (math::equals(ires1, 10));
  }
...
//  std::cout<<"dotproduct: "<<dotproduct<<std::endl;
  ...
  // validate
  validate (math::equals(normVec1,   1.502540907218387));
  validate (math::equals(normVec2,   6.076423472407709));
  validate (math::equals(dotproduct, 7.069617899295469));
	...
	bool partitionedRowWise)
{
	...
	for (int i=0; i < result_local.rows(); i++) {
	 for (int j=0; j < result_local.cols(); j++) {
		 ...
		 {
			 if(!math::equals(result_local(i,j), reference_global(i+off,j)))
			 	std::cout<<"failed: ("<<i+off<<", "<<j<<") result, reference:"<<result_local(i,j)<<", "<<reference_global(i+off,j)<<std::endl;
			 validate (math::equals(result_local(i,j), reference_global(i+off,j)));
		 }else{
			 if(!math::equals(result_local(i,j), reference_global(i,j+off)))
			 	std::cout<<"failed: ("<<i<<", "<<j+off<<") result, reference:"<<result_local(i,j)<<", "<<reference_global(i,j+off)<<std::endl;
			 validate (math::equals(result_local(i,j), reference_global(i,j+off)));
		 }
	 }
	}
}

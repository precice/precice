...
#include "ParallelMatrixOperationsTest.hpp"
...
:
  TestCase ( "cplscheme::tests::ParallelMatrixOperationsTest" )
{}
...
void ParallelMatrixOperationsTest::testParallelMatrixMatrixOp_tarch()
{
	 ...
	 */
	tarch::la::DynamicMatrix<double> W_global (n_global, m_global);
	tarch::la::DynamicMatrix<double> J_global (n_global, n_global);
	tarch::la::DynamicMatrix<double> Z_global (m_global, n_global);
	tarch::la::DynamicMatrix<double> WZ_global (n_global, n_global);
	tarch::la::DynamicMatrix<double> JW_global (n_global, m_global);
	tarch::la::DynamicVector<double> res_global (n_global);
	tarch::la::DynamicVector<double> Jres_global (n_global);

	std::vector<double> J_values(100);
	J_values =  {
		0.162182308193243,   0.450541598502498,   0.106652770180584,   0.431413827463545,   0.853031117721894,   0.417267069084370,   0.780252068321138,   0.234779913372406,   0.547008892286345,   0.929385970968730,
	...
	std::vector<double> Z_values(50);
	Z_values =  {
		0.644318130193692,   0.939001561999887,   0.207742292733028,   0.194764289567049,   0.311102286650413,   0.979748378356085,   0.594896074008614,   0.117417650855806,   0.085515797090044,   0.730330862855453,
	...
	std::vector<double> W_values(50);
	W_values =  {
		0.963088539286913,   0.037738866239552,   0.106761861607241,   0.030540946304637,   0.182922469414914,
	...
	std::vector<double> WZ_values(100);
	WZ_values =  {
		0.801898401838153,   1.122529091861272,   0.423201945415435,   0.300978558234156,   0.551563616222783,   1.054655768517680,   0.709477478632269,   0.264166315674375,   0.348583586200086,   0.874757870918319,
	...
	std::vector<double> JW_values(50);
	JW_values =  {
		2.977456624461038,   2.205533591974792,   3.027360939693343,   3.287118956132616,   2.375186418688407,
	...
	std::vector<double> res_values(10);
	res_values = {
		0.422885689100085,
		0.094229338887735,
		0.598523668756741,
	...
	std::vector<double> Jres_values(10);
	Jres_values = {
		2.104516338543882,
		2.345060145175945,
		1.506380184916943,
	...
	/**
	 * copy values into Matrix objects
	 */
	for(int i = 0; i < n_global; i++)
		for (int j = 0; j < n_global; j++){
			J_global(i,j) = J_values[i * n_global + j];
			WZ_global(i,j) = WZ_values[i * n_global + j];
		}

	for(int i = 0; i < n_global; i++)
		for (int j = 0; j < m_global; j++){
			W_global(i,j) = W_values[i * m_global + j];
			JW_global(i,j) = JW_values[i * m_global + j];
		}

	for(int i = 0; i < m_global; i++)
		for (int j = 0; j < n_global; j++){
			Z_global(i,j) = Z_values[i * n_global + j];
		}

	for(int i = 0; i < n_global; i++){
		res_global(i) = res_values[i];
		Jres_global(i) = Jres_values[i];
	}
	...
	tarch::la::DynamicMatrix<double> W_local (n_local, m_global);
	tarch::la::DynamicMatrix<double> J_local (n_global, n_local);
	tarch::la::DynamicMatrix<double> Z_local (m_global, n_local);
	tarch::la::DynamicMatrix<double> WZ_local (n_global, n_local);
	tarch::la::DynamicMatrix<double> JW_local (n_local, m_global);
	tarch::la::DynamicVector<double> res_local (n_local);
	tarch::la::DynamicVector<double> Jres_local (n_local);
	...
	tarch::la::DynamicMatrix<double> resJW_local (n_local, m_global);
	...
	tarch::la::DynamicMatrix<double> resWZ_local (n_global, n_local);
	...
	tarch::la::DynamicVector<double> resJres_local (n_local);
	parMatrixOps.multiply(J_local, res_local, resJres_local, vertexOffsets, n_global, n_global);
	validate_result_equals_reference(resJres_local, Jres_global, vertexOffsets);
...
void ParallelMatrixOperationsTest::testParallelMatrixMatrixOp_Eigen()
{
	preciceTrace ( "testParallelMatrixMatrixOp_tarch" ); assertion ( utils::Parallel::getCommunicatorSize() == 4 );
...
void ParallelMatrixOperationsTest::validate_result_equals_reference(
	...
	bool partitionedRowWise)
{
	...
	for (int i=0; i < result_local.rows(); i++) {
	 for (int j=0; j < result_local.cols(); j++) {
		 ...
		 {
			 if(!tarch::la::equals(result_local(i,j), reference_global(i+off,j)))
			 	std::cout<<"failed: ("<<i+off<<", "<<j<<") result, reference:"<<result_local(i,j)<<", "<<reference_global(i+off,j)<<std::endl;
			 validate (tarch::la::equals(result_local(i,j), reference_global(i+off,j)));
		 }else{
			 if(!tarch::la::equals(result_local(i,j), reference_global(i,j+off)))
			 	std::cout<<"failed: ("<<i<<", "<<j+off<<") result, reference:"<<result_local(i,j)<<", "<<reference_global(i,j+off)<<std::endl;
			 validate (tarch::la::equals(result_local(i,j), reference_global(i,j+off)));
		 }
	 }
	}
}
...
{
	int off = offsets[utils::MasterSlave::_rank];
	for (int i=0; i < result_local.size(); i++) {
		if(!tarch::la::equals(result_local(i), reference_global(i+off)))
			std::cout<<"failed at "<<i+off<<", result, reference:"<<result_local(i)<<", "<<reference_global(i+off)<<std::endl;
		validate (tarch::la::equals(result_local(i), reference_global(i+off)));
	}
}

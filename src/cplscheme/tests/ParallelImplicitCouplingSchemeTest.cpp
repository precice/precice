...
BOOST_AUTO_TEST_CASE(testInitializeData)
{
  ...
  if (context.isNamed(nameParticipant0)) {
    ...
    sendCouplingData->setSampleAtTime(time::Storage::WINDOW_START, time::Sample{1, Eigen::VectorXd::Constant(1, 4.0)});
    ...
    Eigen::VectorXd v(3);
    v << 1.0, 2.0, 3.0;
    sendCouplingData->setSampleAtTime(time::Storage::WINDOW_START, time::Sample{3, v});
    cplScheme.markActionFulfilled(CouplingScheme::Action::InitializeData);
...
BOOST_AUTO_TEST_CASE(FirstOrder)
{
  ...
  cplData->setSampleAtTime(time::Storage::WINDOW_START, cplData->sample()); // data provided for initial value
  ...
  cplData->setSampleAtTime(time::Storage::WINDOW_END, cplData->sample()); // data provided at end of first window
  ...
  cplData->setSampleAtTime(time::Storage::WINDOW_END, cplData->sample()); // data provided at end of second window
  Fixture::setTimeWindows(scheme, scheme.getTimeWindows() + 1);
  ...
  cplData->setSampleAtTime(time::Storage::WINDOW_END, cplData->sample()); // data provided at end of third window
  Fixture::setTimeWindows(scheme, scheme.getTimeWindows() + 1);
...
BOOST_AUTO_TEST_CASE(FirstOrderWithInitializationAndAcceleration)
{
  ...
  for (int i = 0; i < maxIterations; i++) {
    ...
    mesh->data(sendDataIndex)->setSampleAtTime(time::Storage::WINDOW_END, time::Sample{1, Eigen::VectorXd::Constant(1, 3.0)});
    cplScheme.addComputedTime(timeStepSize);

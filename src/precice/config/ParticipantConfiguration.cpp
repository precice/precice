    ...
    const impl::PtrParticipant &     participant)
{
  ...
  for (const ConfMapping &confMapping : _mappingConfig->mappings()) {
    ...
    if (context.size > 1) {
      ...
      } else if (confMapping.mapping->getConstraint() == mapping::Mapping::SCALED_CONSISTENT_SURFACE || confMapping.mapping->getConstraint() == mapping::Mapping::SCALED_CONSISTENT_VOLUME) {

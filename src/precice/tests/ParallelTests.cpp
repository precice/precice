...
BOOST_AUTO_TEST_CASE(NearestProjectionRePartitioning, * testing::OnSize(4))
{
  ...
  if(utils::Parallel::getProcessRank()<=2){
    ...
    if(utils::Parallel::getProcessRank()==1){
      ...
      BOOST_TEST(positions.size() == numberOfVertices * dimensions);
      interface.setMeshVertices(meshID, numberOfVertices, positions.data(), vertexIDs.data());
    }

    interface.initialize();

    //after re-partitioning, there should be 0 triangles on rank 0 (look at vtk output)
    if(utils::Parallel::getProcessRank()==0){
      BOOST_TEST(impl(interface).getMeshHandle("Nodes").triangles().size()==0);
    }

    //after re-partitioning, there should be 15 triangles on rank 1 (look at vtk output)
    if(utils::Parallel::getProcessRank()==1){
    ...
    // The SolidSolver Provides the Nodes mesh only
    ...
    for( int i = 0; i < numberOfCells; i++){
      ...
      interface.setMeshTriangle(meshID, edgeIDs[4*i+1], edgeIDs[4*i+4], edgeIDs[4*i+3]); //top-right-diag
    }
    BOOST_TEST(impl(interface).getMeshHandle("Nodes").triangles().size()==32);

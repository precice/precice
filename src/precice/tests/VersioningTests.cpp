...
BOOST_AUTO_TEST_CASE(VersionMacroGTE)
{
  PRECICE_TEST(1_rank);

  BOOST_REQUIRE(PRECICE_VERSION_GTE(0, 0, 0));
  BOOST_REQUIRE(PRECICE_VERSION_GTE(1, 0, 0));
  BOOST_REQUIRE(PRECICE_VERSION_GTE(1, 1, 1));

  // Equality
  BOOST_REQUIRE(PRECICE_VERSION_GTE(
      PRECICE_VERSION_MAJOR,
      PRECICE_VERSION_MINOR,
      PRECICE_VERSION_PATCH));

  // Wild guess
  BOOST_REQUIRE(!PRECICE_VERSION_GTE(99, 9, 99));
}

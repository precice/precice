...
void RequestManager:: handleRequests()
{
  ...
  P_DEBUG("ClientCommSize " << clientCommSize);
  ...
  while(true){
    if((std::find(clientRanks.begin(), clientRanks.end(), rankSender) == clientRanks.end()) &&
      ...
      P_CHECK(requestID != -1, "Receiving of request ID failed");
      P_DEBUG("Received request ID " << requestID << " from rank " << rankSender);
    }
    else{
      rankSender++;
    ...
    case REQUEST_PING:
      ...
      break;
    default:
      P_ERROR("Unknown RequestID \"" << requestID << "\"");
      break;
    }
...
(
  ...
  int fourthEdgeID )
{
  P_TRACE(meshID, firstEdgeID, secondEdgeID, thirdEdgeID, fourthEdgeID);
...
(
  ...
  int fourthVertexID )
{
  P_TRACE(meshID, firstVertexID, secondVertexID, thirdVertexID, fourthVertexID);
  ...
  int rankSender )
{
  ...
  P_CHECK(size > 0, "You cannot call setMeshVertices with size=0.");

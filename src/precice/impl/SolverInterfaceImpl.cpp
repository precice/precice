...
{
  P_CHECK(!_accessorName.empty(), "Accessor has to be named!");
  P_CHECK(_accessorProcessRank >= 0, "Accessor process index has to be >= 0!");
  P_CHECK(_accessorCommunicatorSize >= 0, "Accessor process size has to be >= 0!");
  P_CHECK(_accessorProcessRank < _accessorCommunicatorSize,
  ...
  const std::string& configurationFileName )
{
  ...
  if(_accessorProcessRank==0){
    P_INFO("This is preCICE version " << PRECICE_VERSION);
    P_INFO("Revision info: " << precice::preciceRevision);
    P_INFO("Configuring preCICE with configuration: \"" << configurationFileName << "\"" );
  }
  ...
  const config::SolverInterfaceConfiguration& config )
{
  ...
  P_CHECK(not (_accessor->useServer() && _accessor->useMaster()), "You cannot use a server and a master.");
  P_CHECK(_accessorCommunicatorSize==1 || _accessor->useMaster() || _accessor->useServer(),
  ...
  if (_serverMode){
    P_INFO("Run in server mode");
  }
  if (_clientMode){
    P_INFO("Run in client mode");
  }

  if (not _clientMode){
    P_INFO("Run in coupling mode");
    P_CHECK(_participants.size() > 1, "At least two participants need to be defined!");
...
double SolverInterfaceImpl:: initialize()
{
  ...
  if (_clientMode){
    P_DEBUG("Request perform initializations");
    _requestManager->requestInitialize();
  }
  else {
    // Setup communication

    P_INFO("Setting up master communication to coupling partner/s" );
    for (auto& m2nPair : _m2ns) {
        auto& bm2n = m2nPair.second;
        P_DEBUG((bm2n.isRequesting?"Awaiting master connection from ":"Establishing master connection to ") << bm2n.remoteName);
        bm2n.prepareEstablishment();
        bm2n.connectMasters();
        P_DEBUG("Established master connection " << (bm2n.isRequesting?"from ":"to ") << bm2n.remoteName);
    }
    P_INFO("Masters are connected");

    computePartitions();

    P_INFO("Setting up slaves communication to coupling partner/s" );
    for (auto& m2nPair : _m2ns) {
      auto& bm2n = m2nPair.second;
      P_DEBUG((bm2n.isRequesting?"Awaiting slaves connection from ":"Establishing slaves connection to ") << bm2n.remoteName);
      bm2n.connectSlaves();
      bm2n.cleanupEstablishment();
      P_DEBUG("Established slaves connection " << (bm2n.isRequesting?"from ":"to ") << bm2n.remoteName);
    }
    P_INFO("Slaves are connected");
    ...
    P_INFO(_couplingScheme->printCouplingState());
  }
...
void SolverInterfaceImpl:: initializeData ()
{
  ...
  P_CHECK(_couplingScheme->isInitialized(),
...
{
  P_TRACE(computedTimestepLength);
  ...
  P_CHECK(_couplingScheme->isInitialized(), "initialize() has to be called before advance()");
  _numberAdvanceCalls++;
  if (_clientMode){
...
#   endif
    ...
    P_DEBUG("Advancing coupling scheme");
...
void SolverInterfaceImpl:: finalize()
{
  ...
  P_CHECK(_couplingScheme->isInitialized(), "initialize() has to be called before finalize()");
...
{
  P_TRACE(action, _couplingScheme->isActionRequired(action));
  ...
  const std::string& meshName ) const
{
  ...
  P_CHECK(pos != _meshIDs.end(), "Mesh with name \""<< meshName << "\" is not defined!" );
  return pos->second;
}
  ...
  const std::string& dataName, int meshID ) const
{
  P_TRACE(dataName, meshID );
  PRECICE_VALIDATE_MESH_ID(meshID);
  P_CHECK(hasData(dataName, meshID),
  ...
  int meshID )
{
  ...
  if (_clientMode){
    ...
    P_CHECK(!isStationary, "A mesh with only initial mappings  must not be reseted");
    P_CHECK(hasMapping, "A mesh with no mappings must not be reseted");

    P_DEBUG( "Clear mesh positions for mesh \"" << context.mesh->getName() << "\"" );
  ...
  const double* position )
{
  ...
  P_DEBUG("Position = " << internalPosition);
  int index = -1;
  if ( _clientMode ){
    ...
    P_DEBUG("MeshRequirement: " << context.meshRequirement);
  ...
  int*          ids ) const
{
  ...
  if (_clientMode){
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      P_CHECK(j != vsize, "Position " << i << "=" << ids[i] << " unknown!");
      ids[i] = j;
    }
  }
}
...
(
  ...
  int secondVertexID )
{
  P_TRACE(meshID, firstVertexID, secondVertexID );
  if ( _clientMode ){
    ...
    if ( context.meshRequirement == mapping::Mapping::MeshRequirement::FULL ){
      P_DEBUG("Full mesh required.");
      mesh::PtrMesh& mesh = context.mesh;
      P_CHECK(mesh->isValidVertexID(firstVertexID),  " Given VertexID is invalid!");
      P_CHECK(mesh->isValidVertexID(secondVertexID), " Given VertexID is invalid!");
  ...
  int thirdEdgeID )
{
  ...
  if ( _clientMode ){
    ...
    if ( context.meshRequirement == mapping::Mapping::MeshRequirement::FULL ){
      mesh::PtrMesh& mesh = context.mesh;
      P_CHECK(mesh->isValidEdgeID(firstEdgeID),  " Given EdgeID is invalid!");
      P_CHECK(mesh->isValidEdgeID(secondEdgeID), " Given EdgeID is invalid!");
      P_CHECK(mesh->isValidEdgeID(thirdEdgeID),  " Given EdgeID is invalid!");
  ...
  int thirdVertexID )
{
  ...
  if (context.meshRequirement == mapping::Mapping::MeshRequirement::FULL){
    ...
    P_CHECK(mesh->isValidVertexID(firstVertexID),  " Given VertexID is invalid!");
    P_CHECK(mesh->isValidVertexID(secondVertexID), " Given VertexID is invalid!");
    P_CHECK(mesh->isValidVertexID(thirdVertexID),  " Given VertexID is invalid!");
...
(
  ...
  int fourthEdgeID )
{
  P_TRACE(meshID, firstEdgeID, secondEdgeID, thirdEdgeID,
                fourthEdgeID);
  if (_clientMode){
    ...
    if (context.meshRequirement == mapping::Mapping::MeshRequirement::FULL){
      ...
      P_CHECK(mesh->isValidEdgeID(secondEdgeID), " Given EdgeID is invalid!");
      P_CHECK(mesh->isValidEdgeID(thirdEdgeID),  " Given EdgeID is invalid!");
      P_CHECK(mesh->isValidEdgeID(fourthEdgeID), " Given EdgeID is invalid!");
      mesh::Edge& e0 = mesh->edges()[firstEdgeID];
      mesh::Edge& e1 = mesh->edges()[secondEdgeID];
      mesh::Edge& e2 = mesh->edges()[thirdEdgeID];
  ...
  int fourthVertexID )
{
  ...
  if (context.meshRequirement == mapping::Mapping::MeshRequirement::FULL){
    ...
    P_CHECK(mesh->isValidVertexID(secondVertexID), " Given VertexID is invalid!");
    P_CHECK(mesh->isValidVertexID(thirdVertexID),  " Given VertexID is invalid!");
    P_CHECK(mesh->isValidVertexID(fourthVertexID), " Given VertexID is invalid!");
  ...
  int fromMeshID )
{
  ...
  if (mappingContext.mapping.use_count() == 0){
    P_ERROR("From mesh \"" << context.mesh->getName()
                   << "\", there is no mapping defined");
    return;
  }
  if (not mappingContext.mapping->hasComputedMapping()){
    P_DEBUG("Compute mapping from mesh \"" << context.mesh->getName() << "\"");
...
(
  ...
  const double* value )
{
  P_TRACE(fromDataID, valueIndex );
  PRECICE_VALIDATE_DATA_ID(fromDataID);
  P_DEBUG("value = " << Eigen::Map<const Eigen::VectorXd>(value, _dimensions));
  if (_clientMode){
...
(
  ...
  double value)
{
  P_TRACE(fromDataID, valueIndex, value );
  PRECICE_VALIDATE_DATA_ID(fromDataID);
  P_CHECK(valueIndex >= -1, "Invalid value index (" << valueIndex << ") when writing scalar data!");
  if (_clientMode){
...
(
  ...
  double* value ) const
{
  P_TRACE(toDataID, valueIndex);
  PRECICE_VALIDATE_DATA_ID(toDataID);
  P_CHECK(valueIndex >= -1, "Invalid value index ( " << valueIndex << " )when reading vector data!");
  if (_clientMode){
  ...
  P_DEBUG("read value = " << Eigen::Map<const Eigen::VectorXd>(value, _dimensions));
}
  ...
  double*    values ) const
{
  ...
  if (_clientMode){
    ...
    for (int i=0; i < size; i++){
        const auto valueIndex = valueIndices[i];
        P_CHECK(0 <= valueIndex && valueIndex < valuesInternal.size(), "Value index out of range");
...
(
  ...
  double& value ) const
{
  P_TRACE(toDataID, valueIndex, value);
  PRECICE_VALIDATE_DATA_ID(toDataID);
  P_CHECK(valueIndex >= -1, "Invalid value index ( " << valueIndex << " )when reading vector data!");
  if (_clientMode){
...
{
  P_TRACE(filenameSuffix, exportType );
  // Export meshes
  //const ExportContext& context = _accessor->exportContext();
  for (const io::ExportContext& context : _accessor->exportContexts()) {
    P_DEBUG( "Export type = " << exportType );
    ...
    if ( exportAll || exportThis ){
      for (const MeshContext* meshContext : _accessor->usedMeshContexts()) {
        ...
        P_DEBUG( "Exporting mesh to file \"" << name << "\" at location \"" << context.location << "\"" );
  ...
  const std::string& meshName )
{
  ...
  P_ERROR("Participant \"" << _accessorName
  ...
  const m2n::M2NConfiguration::SharedPointer& m2nConfig )
{
  ...
  for (MeshContext* context : _accessor->usedMeshContexts()) {
    if ( context->provideMesh ) { // Accessor provides mesh
      P_CHECK( context->receiveMeshFrom.empty(),
      ...
      for (auto& receiver : _participants ) {
        for (auto& receiverContext : receiver->usedMeshContexts()) {
          if(receiverContext->receiveMeshFrom == _accessorName && receiverContext->mesh->getName() == context->mesh->getName()){
            //P_CHECK( not hasToSend, "Mesh " << context->mesh->getName() << " can currently only be received once.")
...
void SolverInterfaceImpl:: mapWrittenData()
{
  ...
  for (impl::MappingContext& context : _accessor->writeMappingContexts()) {
    ...
    if (rightTime && not hasComputed){
      P_INFO("Compute write mapping from mesh \""
  ...
  for (impl::DataContext& context : _accessor->writeDataContexts()) {
    ...
    if (hasMapping && rightTime && (not hasMapped)){
      ...
      P_DEBUG("Map data \"" << context.fromData->getName()
                   << "\" from mesh \"" << context.mesh->getName() << "\"");
      context.toData->values() = Eigen::VectorXd::Zero(context.toData->values().size());
      P_DEBUG("Map from dataID " << inDataID << " to dataID: " << outDataID);
...
void SolverInterfaceImpl:: mapReadData()
{
  ...
  for (impl::MappingContext& context : _accessor->readMappingContexts()) {
    ...
    if (mapNow && not hasComputed){
      P_INFO("Compute read mapping from mesh \""
              << _accessor->meshContext(context.fromMeshID).mesh->getName()
              << "\" to mesh \""
              << _accessor->meshContext(context.toMeshID).mesh->getName()
              << "\".");
  ...
  for (impl::DataContext& context : _accessor->readDataContexts()) {
    ...
    if (mapNow && hasMapping && (not hasMapped)){
      ...
      int outDataID = context.toData->getID();
      context.toData->values() = Eigen::VectorXd::Zero(context.toData->values().size());
      P_DEBUG("Map read data \"" << context.fromData->getName()
                   << "\" to mesh \"" << context.mesh->getName() << "\"");
      context.mappingContext.mapping->map(inDataID, outDataID);
      P_DEBUG("First mapped values = " << utils::firstN(context.toData->values(), 10));
    }
   ...
   const config::SolverInterfaceConfiguration& config )
{
  ...
  P_ERROR("Accessing participant \"" << _accessorName << "\" is not defined in configuration!");
}
...
void SolverInterfaceImpl:: initializeClientServerCommunication()
{
  ...
  if ( _serverMode ){
    ...
    P_INFO("Setting up communication to server" );
...
void SolverInterfaceImpl:: initializeMasterSlaveCommunication()
{
  ...
  if ( utils::MasterSlave::isMaster() ){
    P_INFO("Setting up communication to slaves" );
...
void SolverInterfaceImpl:: syncTimestep(double computedTimestepLength)
{
  ...
  if(utils::MasterSlave::isSlave()){
    ...
    for(int rankSlave = 1; rankSlave < _accessorCommunicatorSize; rankSlave++){
      ...
      P_CHECK(math::equals(dt, computedTimestepLength),

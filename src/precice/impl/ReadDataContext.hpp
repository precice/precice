...
/**
 * @brief Stores one Data object with related mesh. Context stores data to be read from and potentially provides a read
 * mapping. Additionally stores Waveform object associated with _providedData.
 *
...
public:
  ...
  /**
   * @brief Adds a MappingContext and the MeshContext required by the read mapping to the correspnding ReadDataContext
   * data structures.
   *
   * A read mapping maps _fromData to _providedData. A ReadDataContext already has _providedData, but additionally
   * requires _fromData.
   *
   ...
   *
   * @note Note that read mapping contexts have to be unique and and this function may only be called once for a given
   * ReadDataContext
   */
   ...
   *
   * @param normalizedDt Point in time where waveform is sampled. Must be normalized to [0,1], where 0 refers to the
   * beginning and 1 to the end of the current time window.
   */
